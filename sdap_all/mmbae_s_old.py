...
Ref: Modofied from LDBAE code by Clinton Jones
'''
...
def learn_bae(Y, W, Xs, K, L, num_runs, num_iter, precision, reg_lambda, initial_T=50, eta=0.5, init_point=None):
    ...
    for run in range(num_runs): 
        ...
        while delta > precision and t < num_iter:
            # M-step
            print "M STEP"
        ...
        bae_parameters[run]['log_likelihood'] = log_likelihood[t-1]
    ...
    if D1 > 0:
        for k in range(K):
            if (np.sum(r_m0k[:,k])>1e-10):
                ...
                assert ((mu_x_tiled[1,:]==mu_x_tiled[2,:]).all)
                x_minus_mu_square = (Xs[0]**2 - 2*Xs[0]*mu_x_tiled + mu_x_tiled**2).reshape(M,D1)
    ...
    if D2 > 0:
        for l in range(L):
            if (np.sum(r_n0l[:,l])>1e-10):
                ...
                x_minus_mu_square = (Xs[1]**2 -2*Xs[1]*mu_x_tiled+ mu_x_tiled**2).reshape(N,D2)
...
def update_alphas(gamma_mk, gamma_nl, alphas_old, I, J, debug=False):
    ...
    if (debug==True):
        v1_old,g=objective_value(alphas_old[0],gamma_mk)
        v1,g=objective_value(alphas_new[0],gamma_mk)
        v1=-v1
        v1_old=-v1_old
        if ((v1_old-v1)>1e-5):
...
def update_mean_field_rs(Xs, mus_X, sigmas_X, betas, sigmas_Y, gamma_mk, gamma_nl, Y, r_nml, r_mnk, T, I, J):
    ...
    # r_m0k update
    for k in range(K):
        x_minus_mu_square = (Xs[0]**2 -2*Xs[0]*np.tile(mus_X[0][k,:], (M, 1))+ np.tile(mus_X[0][k,:], (M, 1))**2).reshape(M,D1) # M x D1 (x_1m-mu_1)
