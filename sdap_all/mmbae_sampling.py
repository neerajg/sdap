...
def learn_bae(Y, W, Xs, K, L, num_runs, num_iter, precision, reg_lambda, initial_T=50, eta=0.5, init_point=None):
    ...
    bae_parameters = []
    log_likelihood = np.zeros(num_iter/10)
    ...
    for run in range(num_runs): 
        ...
        while t < num_iter:#delta > precision*20  
            # M-step
            print "M STEP"
            ...
            #print_lowerboud(Y, Xs, alphas, mus_X, sigmas_X, betas, sigmas_Y, r_mnk, r_nml, r_m0k, r_n0l, gamma_mk, gamma_nl, I, J, reg_lambda)
            if (t%100==0):     
                log_likelihood[a] = get_lower_bound(Y, Xs, alphas, mus_X, sigmas_X, betas, sigmas_Y, r_mnk, r_nml, r_m0k, r_n0l, gamma_mk, gamma_nl, I, J, reg_lambda)
                if log_likelihood[a] == np.nan:
                    print "Invalid likelihood (happens occasionally)!  Dropping to debugger"
                    pdb.set_trace()
                if a>0:
                    delta = log_likelihood[a]-log_likelihood[a-1]
                print "\t%s: Iteration %d(%d), lower bound: %f (delta: %f)" % (datetime.now(), t, a, log_likelihood[a], delta)
                a=a+1
            ...
            # E-step (mean field)
            print "E STEP"  
            r_mnk_1, r_nml_1, r_m0k_1, r_n0l_1, gamma_mk, gamma_nl = mean_field(Y, I, J, Xs, alphas, mus_X, sigmas_X, betas, sigmas_Y, r_mnk_1, r_nml_1, r_m0k_1, r_n0l_1, initial_T, eta)
            
            
            r_mnk=np.array([np.random.multinomial(1,r_mnk_1[nnz,:]) for nnz in range(num_nonzero)])
            #print r_mnk.shape
            r_nml=np.array([np.random.multinomial(1,r_nml_1[nnz,:]) for nnz in range(num_nonzero)])
            r_m0k=np.array([np.random.multinomial(1,r_m0k_1[m,:]) for m in range(M)])
            r_n0l=np.array([np.random.multinomial(1,r_n0l_1[n,:]) for n in range(N)])
            #print_lowerboud(Y, Xs, alphas, mus_X, sigmas_X, betas, sigmas_Y, r_mnk, r_nml, r_m0k, r_n0l, gamma_mk, gamma_nl, I, J, reg_lambda)
            gc.collect()
        ...
        bae_parameters[run]['log_likelihood'] = log_likelihood[a-1]
        ...
        if log_likelihood[a-1] > best_log_likelihood:
            best_log_likelihood = log_likelihood[a-1]
            best_run = run
    ...
    if num_runs == 1:
        best_run = 0
    return bae_parameters, best_run, init_point, log_likelihood
    ...
    if D1 > 0:
        for k in range(K):
            if (np.sum(r_m0k[:,k])>1e-10):
                mus_X[0][k,:] = np.sum(np.tile(r_m0k[:,k].reshape(M,1),(1,D1))*Xs[0],0) / np.sum(r_m0k[:,k])
                mu_x_tiled=np.tile(mus_X[0][k,:].reshape(1,D1), (M, 1))
                x_minus_mu_square = (Xs[0]**2 - 2*Xs[0]*mu_x_tiled + mu_x_tiled**2)
                sigmas_X[0][k] = np.sum((r_m0k[:,k]*(np.sum(x_minus_mu_square, axis=1)))/(D1*np.sum(r_m0k[:,k]))) 
            else:
    ...
    if D2 > 0:
        for l in range(L):
            if (np.sum(r_n0l[:,l])>1e-10):
                mus_X[1][l,:] = np.sum(np.tile(r_n0l[:,l].reshape(N,1),(1,D2))*Xs[1],0) / np.sum(r_n0l[:,l])
                mu_x_tiled=np.tile(mus_X[1][l,:].reshape(1,D2), (N, 1))
                x_minus_mu_square = (Xs[1]**2 -2*Xs[1]*mu_x_tiled+ mu_x_tiled**2)
                sigmas_X[1][l] = np.sum( (r_n0l[:,l]*(np.sum(x_minus_mu_square, axis=1)))/(D2*np.sum(r_n0l[:,l])))
            else:
    ...
    for k in range(K):
        ...
        for l in range(L):
            ...
            if ((linear_weights>0.01).any):
                sqrt_weights = np.sqrt(linear_weights)
                ...
                numerator = np.sum(weights*(Ysq_values + new_beta_times_x**2 - 2*Y_values*new_beta_times_x))+reg_lambda*np.sum(betas_new[k,l,:]**2)
...
def mean_field(Y, I, J, Xs, alphas, mus_X, sigmas_X, betas, sigmas_Y, r_mnk, r_nml, r_m0k, r_n0l, initial_T, eta):
    ...
    delta = 1e99
    iters_run = 0
    while delta > 1e-3 and iters_run<1000:
        # Update gammas
...
def update_mean_field_rs(Xs, mus_X, sigmas_X, betas, sigmas_Y, gamma_mk, gamma_nl, Y, r_nml, r_mnk, T, I, J):
    ...
    for k in range(K):
        ...
        log_r_m0k_new[:,k] += ((-.5*D1)*(np.log(2*np.pi*sigmas_X[0][k])) - (.5/sigmas_X[0][k])*np.sum(x_minus_mu_square, axis=1)).reshape(M,)
        # psi(gamma_k[k])
    ...
    # r_n0l update
    for l in range(L):
        x_minus_mu_square = (Xs[1]**2 -2*Xs[1]*np.tile(mus_X[1][l,:], (N, 1))+ np.tile(mus_X[1][l,:], (N, 1))**2).reshape(N,D2) # N x D2 (x_2n-mu_2)
        # p(x|theta) is gaussian, mean mus_X[k], variance sigmas_X
        log_r_n0l_new[:,l] += ((-.5*D2)*(np.log(2*np.pi*sigmas_X[1][l])) - (.5/sigmas_X[1][l])*np.sum(x_minus_mu_square, axis=1)).reshape(N,)
        # psi(gamma_k[k])
    ...
    for k in range(K):
        ...
        for l in range(L):
            ...
            Y_values = np.array(Y[(I,J)]).reshape((num_nonzero,))
            expected_log_prob = -.5*np.log(2*np.pi*sigmas_Y[k,l]) - (.5/sigmas_Y[k,l])*(Y_values**2- 2*Y_values*beta_times_x +beta_times_x**2)
            log_r_mnk_new[:,k] += expected_log_prob * r_nml[:,l]
            log_r_nml_new[:,l] += expected_log_prob * r_mnk[:,k]
    del r_mnk, r_nml 
    
    log_r_m0k_new = log_r_m0k_new/T
    #log_r_m0k_new = log_r_m0k_new
    
    log_r_mnk_new = log_r_mnk_new/T
    #log
    _r_mnk_new = log_r_mnk_new
    
    log_r_n0l_new = log_r_n0l_new/T
    #log_r_n0l_new = log_r_n0l_new
    
    log_r_nml_new = log_r_nml_new/T
...
#    for k in range(K):
#        psi_gamma_helpers = np.array([psi_gamma_helper(gamma_mk[m,:], k) for m in range(M)].reshape(M,))
#        # Expectation of log[p(pi_1m)] continued
#        alpha_times_psi_stuff = (alphas[0][k]-1)*psi_gamma_helpers
#        log_likelihood_lower_bound += np.sum(alpha_times_psi_stuff)
#        # E_q[log(p(z_1m))]
#        log_likelihood_lower_bound += np.sum(r_m0k[:,k].reshape(M,)*psi_gamma_helpers)
#        
#        # H[q(pi_1m)]
...
#        log_likelihood_lower_bound -= np.sum(gamma_times_psi_stuff)
    ...
    for k in range(K):
        x_minus_mu_square =(Xs[0]**2 -2*(Xs[0]*np.tile(mus_X[0][k,:], (M, 1)))+ np.tile(mus_X[0][k,:], (M, 1))**2)
        expectation_logx=(r_m0k[:,k]*(((-.5*D1)*np.log(2*np.pi*(sigmas_X[0][k])) - (.5/sigmas_X[0][k])*np.sum(x_minus_mu_square, axis=1)))).reshape(M,1)
        log_likelihood_lower_bound += np.sum(expectation_logx)
        
    # Terms involving summation over N   
    ...
    for l in range(L):
        ...
        expectation_logx=(r_n0l[:,l]*(((-.5*D2)*np.log(2*np.pi*(sigmas_X[1][l])) - (.5/sigmas_X[1][l])*np.sum(x_minus_mu_square, axis=1)))).reshape(N,1)
        log_likelihood_lower_bound += np.sum(expectation_logx)
