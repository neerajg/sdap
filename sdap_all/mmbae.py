...
def learn_bae(Y, W, Xs, K, L, num_runs, num_iter, precision, reg_lambda, initial_T=50, eta=0.5, init_point=None):
    ...
    for run in range(num_runs): 
        ...
        while delta > precision*100 and t < num_iter:
            # M-step
            print "M STEP"
            ...
            # E-step (mean field)
            print "E STEP"  
            r_mnk, r_nml, r_m0k, r_n0l, gamma_mk, gamma_nl = mean_field(Y, I, J, Xs, alphas, mus_X, sigmas_X, betas, sigmas_Y, r_mnk_old, r_nml_old, r_m0k_old, r_n0l_old, initial_T, eta)
            #print_lowerboud(Y, Xs, alphas, mus_X, sigmas_X, betas, sigmas_Y, r_mnk, r_nml, r_m0k, r_n0l, gamma_mk, gamma_nl, I, J, reg_lambda)
            gc.collect()
...
def init_alphas(r_mnk, r_nml, r_m0k, r_n0l):
    ...
    alphas[0] = (np.mean(r_mnk,0)+np.mean(r_m0k,0))/2
    alphas[1] = (np.mean(r_nml,0)+np.mean(r_n0l,0))/2
    return alphas
...
def update_theta(r_m0k, r_n0l, Xs, mus_X_old, sigmas_X_old, debug=False):
    ''' 
    Updates the values of mu_X and sigma_X which are matrices of dimensions [(K x D1),(L x D2)] and [(K X 1),(L X 1)] respectively
    '''
    M,K = r_m0k.shape
    N,L = r_n0l.shape
    try:
...
def update_betas_and_sigmas_Y(Y, I, J, Xs, r_mnk, r_nml, reg_lambda, betas_old, sigmas_Y_old,debug=False):
    ...
    # X is of dimension [num_nonzero x (1+D1+D2)]1
