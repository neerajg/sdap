...
from BAE.misc import initialize
import numpy as np
from scipy.special import psi as digamma
from scipy.special import gammaln
import scipy.sparse as sp
from BAE.misc import objective_fn
import scipy.optimize as opt
from sklearn.linear_model import Ridge

sigma_tolerance = 1e-4

def train_bae_linear(K, L, X1, X2, train_I, train_J, train_Y, reg_beta, num_iter, delta_convg, reg_alpha1, reg_alpha2):
...
def update_betas(Yobs, X_composite, r, betas, reg_beta, K, L, M, N, train_I, train_J):
    ...
    for k in range(K):
        for l in range(L):
            weights = np.multiply(np.tile(np.transpose(r[0][:,k]).reshape(M,1), (1,N)),np.tile(np.transpose(r[1][:,l]).reshape(1,N), (M,1)))[train_I,train_J]
...
def get_log_likelihood_lower_bound(K,L,M,N,alphas,betas,gammas,r,train_I,train_J, train_Y,X_composite):
    ...
    # sum over m and k (r1mk*(digamma(gamma1mk) - digamma(sum over k(gamma1mk))))
    log_likelihood += np.sum(np.multiply(r[0],digamma_gamma1mk)) - np.dot(digamma_gamma1m,np.sum(r[0],1))
    ...
    log_likelihood += np.sum(np.multiply(r[1],digamma_gamma2nl)) - np.dot(digamma_gamma2n,np.sum(r[1],1))
