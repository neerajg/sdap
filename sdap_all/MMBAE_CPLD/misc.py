...
def initialize(X1, X2, train_I, train_J, train_Y, K, L):
    ...
    alphas, gammas, betas, r = init_params(K, L, M, N, X1, X2, no_obs, train_I, train_J)
...
def init_params(K, L, M, N, X1, X2, no_obs, train_I, train_J):
    ...
    gammas[0] = np.tile(alphas[0].reshape(1,K), (M,1)) +  np.multiply(r[0],mu) # M x K
    gammas[1] = np.tile(alphas[1].reshape(1,L), (N,1)) +  np.multiply(r[1],mv) # N x L
