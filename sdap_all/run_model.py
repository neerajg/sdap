...
def runModelHotStart(model_name, K, L, X1, X2, train_I, train_J, train_Y, num_iter, conv_thresh,regs=None):
    ...
    if model[0].upper() == 'SS-SCOAL':
        ...
        if regs==None:
            ...
            reg_op = regs[0]
            reg_ss = regs[1]
        train_op = ss_scoal.run_ssScoal(K, L, X1, X2, train_I, train_J, train_Y, learner,num_iter,conv_thresh,reg_op,reg_ss,semi_sup_learner,semi_supervised=True)
    ...
    if model[0].upper() == 'SCOAL':
        ...
        learner = sub_model
        train_op = ss_scoal.run_ssScoal(K, L, X1, X2, train_I, train_J, train_Y, learner,num_iter,conv_thresh,reg_op,semi_supervised=False)
    '''if model == 'BAE':
...
def runModelColdStart(model_name, K, L, X1, X2, train_I, train_J, train_Y, num_iter, conv_thresh,regs=None):
    # Train the model and return the parameters and objective function
