...
# TO DO : Update row and col assignments by forming huge block diagonal matrices and then calling one ALS update for each row/col         
    ...
    def matrixFactorization_self(self,rows,cols,r,c,algo):
        ...
        Su = sp.csr_matrix((ones, (rows,cols)), shape=(len(set(rows)),len(set(cols)))).sum(1).flatten().tolist()[0]
        Su_root = np.sqrt(Su)
        Sv = sp.csr_matrix((ones, (rows,cols)), shape=(len(set(rows)),len(set(cols)))).sum(0).flatten().tolist()[0]
        Sv_root = np.sqrt(Sv)
        U,V = PMF_self.mf_solver(rows, cols, vals, self.D, self.lambda_reg,Y_sparse.shape[0],Y_sparse.shape[1],Su,Sv,Su_root,Sv_root)
        return U,V
